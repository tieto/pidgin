...
%if 0%{?beta}
%define pidginver %(echo "2.10.0"|sed -e 's/dev.*//; s/beta.*//')
%else
%define pidginver 2.10.0
%endif

# define the minimum API version required, so we can use it for plugin deps
%define apiver %(echo "2.10.0"|awk -F. '{print $1"."$2}')
