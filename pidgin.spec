...
%if 0%{?beta}
%define pidginver %(echo "2.6.5"|sed -e 's/dev.*//; s/beta.*//')
%else
%define pidginver 2.6.5
%endif

# define the minimum API version required, so we can use it for plugin deps
%define apiver %(echo "2.6.5"|awk -F. '{print $1"."$2}')
