...
%if 0%{?beta}
%define pidginver %(echo "2.6.3"|sed -e 's/dev.*//; s/beta.*//')
%else
%define pidginver 2.6.3
%endif

# define the minimum API version required, so we can use it for plugin deps
%define apiver %(echo "2.6.3"|awk -F. '{print $1"."$2}')
